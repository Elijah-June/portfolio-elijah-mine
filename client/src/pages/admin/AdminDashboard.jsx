    ...
    <div className="space-y-4">
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="New Blog Post">
        <form className="space-y-3" onSubmit={createPost}>
          <input className="w-full border border-white/10 bg-white/5 text-white placeholder:text-gray-400 px-3 py-2 rounded" placeholder="Title"
            value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} required />
          <textarea className="w-full border border-white/10 bg-white/5 text-white placeholder:text-gray-400 px-3 py-2 rounded h-48 font-mono text-sm" placeholder="Markdown content"
          ...
          <div className="flex justify-end gap-2">
            <button type="button" className="px-4 py-2 bg-white/10 rounded" onClick={() => setOpen(false)}>Cancel</button>
            <button className="px-4 py-2 bg-white text-black rounded">Create</button>
          </div>
        </form>
      </Modal>
    </div>
