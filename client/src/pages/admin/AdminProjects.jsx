  ...
  return (
    <div className="space-y-6">
      ...
      <form className="grid md:grid-cols-2 gap-3 border border-white/10 rounded p-4 bg-white/5" onSubmit={create}>
        ...
        <textarea className="border border-white/10 bg-white/5 text-white placeholder:text-gray-400 px-3 py-2 rounded md:col-span-2" placeholder="Description" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} />
        {error && <p className="text-red-400 text-sm md:col-span-2">{error}</p>}
        <button className="px-4 py-2 bg-white text-black rounded md:col-span-2">Create Project</button>
      </form>
